...
def selected(selec):
    def manage(aba):
        ...
        def call():
            ...
            # função para abrir o arquivo e escrevendo as informações em data
            def openFile():
                try:
                ...
                try:
                    match aba:
                        case 0 | 1:
                            ...
                            codigo = int(codigo)
                            nome = input(f"{cor.blue}insira o nome do {options[aba + 1]} (max. 45): {cor.white}")[:45].lower()
                            cpf = input(f"{cor.blue}insira o CPF do {options[aba + 1]} (apenas números{cor.reset}{cor.blue}): {cor.white}")
                            if len(cpf) != 11:
                                clear()
